...
## 2026-09-21 — InsChat showcase page

### Solved

- Inspected the latest InChat `sep` branch at `f2ddc74` and verified its current chat, multimodal input, health records, responsive, and model-routing features.
- Added a dedicated `/project/inschat/` case-study page with authentic screenshots from the latest local build, reusable project navigation, responsive sections, reduced-motion support, and stack notes.
- Added InsChat to the homepage project index with verified repository and live-demo links.
- Rebuilt the Gatsby site with Node 16 and audited the page at desktop and 390px mobile widths: four sections render, all three screenshots load, navigation opens, and there is no horizontal overflow.

### Unresolved

- The public `rwkit.com` host could not be resolved from this environment, so verification used the local Gatsby production server and the local InChat production build.

### Disproved

- A generic Markdown-only project page would not show InsChat's actual chat, health-record, and responsive product surfaces clearly enough.
...
## 2026-09-21 — Agent builder workspace showcase

### Solved

- Inspected the latest Agent branch at `3a1346e` and verified its builder-focused workflow: Plan/Build modes, workspace tools, resumable runs, document input, change totals, and commit/push feedback.
- Added a dedicated `/project/agent/` case-study page with authentic desktop, builder-composer, and mobile screenshots from the latest local production build.
- Added Agent to the homepage project index with its verified repository and live-demo links.
- Rebuilt the Agent app successfully with Node 22 before capturing the showcase assets.

### Unresolved

- The local Agent models screen requires the production API environment and returned a request failure, so it was not used as a showcase screenshot.

### Disproved

- Framing Agent as another health-oriented chat would miss its primary value; the case study instead presents it as a prompt-to-change build workspace.

---

## 2026-09-23 — Read-only VM security triage

### Solved

- Checked processes, listening sockets, SSH configuration, login history, persistence locations, systemd services, firewall rules, AppArmor, fail2ban, package verification, web logs, and recent root-owned files without changing the VM.
- Confirmed inbound exposure is limited by active UFW default-deny rules to SSH/HTTP/HTTPS; nginx owns 80/443, sshd owns 22, root SSH/password authentication is disabled, and no successful root SSH login was found.
- Confirmed the observed high CPU processes and outbound connections belong to Cursor services, Oracle Cloud Agent, nginx, or normal system services; no miner, reverse shell, extra account, or unexpected persistence was found.

### Unresolved

- The VM has extensive automated SSH and web probing. A live pre-auth root SSH connection from `60.167.166.161` was observed during the check; it was not authenticated and should expire under the 120-second SSH grace period.
- Eleven historical successful `ubuntu` key authentications came from Oracle-owned IP `147.224.13.78` on 2026-08-25 using the same authorized key as the current user. This is safe only if that cloud host/automation is recognized by the owner.
- Ubuntu, nginx, snapd, cloud-init, and related package updates are pending. `dpkg --verify` reported expected local configuration differences plus one missing monitoring-agent license file; executable integrity was not fully checked because `debsums` is not installed.

### Disproved

- The `sshd: root [priv]` process was not evidence of a successful root session; socket and log inspection identified it as an unauthenticated internet probe.
- The reboot was not linked to an intrusion by the available logs; prior-boot records instead show memory pressure/OOM events involving Chrome and npm processes.

---

## 2026-09-23 — Rebuilt applications and restored PM2 services

### Solved

- Built Agent, InsChat, Profile frontend/backend, and Rencipe frontend/backend successfully.
- Started the requested production services under PM2 using Node 22: `agent`, `inschat`, `profile-backend`, `rencipe-frontend`, and `rencipe-backend`; also restored Agent's available OpenCode and Whisper sidecars.
- Verified all PM2 apps had zero restarts, expected ports were listening, Profile backend returned 200, Rencipe frontend returned 200, and both Rencipe/Agent sidecars were reachable.
- Saved the scoped seven-app PM2 snapshot to `/home/ubuntu/.pm2/dump.pm2`.
- Enabled a user-level PM2 systemd service with lingering for `ubuntu`, because the system unit directory is immutable.

### Unresolved

- The user-level PM2 service is enabled for the next boot but was not started separately during this session; the already-running PM2 daemon was left untouched to avoid duplicate resurrection. A reboot or controlled PM2 restart is still needed to prove boot recovery.
- The Profile frontend is a static Gatsby build served directly by nginx; only its backend runs under PM2.

### Disproved

- PM2 was not removed from the VM. Its CLI remains installed under the Node 22 NVM tree; it was merely absent from the Node 24 `PATH`.
- The `/etc/systemd/system` PM2 installer failure was not a PM2 application failure; the directory has the immutable attribute, so the system-wide unit could not be written.

---

## 2026-09-23 — Muse-inspired InsChat introduction

### Solved

- Reworked the InsChat showcase hero toward a calmer, cinematic introduction: centered promise, dark visual stage, focused primary action, and clearer capability facts.
- Added a final “Try InsChat” closing section with a single live-demo CTA, while preserving the authentic chat, health-record, and mobile screenshots.
- Extended the project navigation to include the new closing CTA.

### Unresolved

- The updated composition still needs a fresh visual browser check at desktop and phone widths after the production build.

### Disproved

- Replacing the existing product evidence with a purely decorative landing page would have weakened the case study; the Muse influence was applied to hierarchy and rhythm instead.

---

## 2026-09-23 — InsChat showcase section UI pass

### Solved

- Restyled the health workflow and build-notes sections with the darker homepage language, higher contrast, stronger screenshot staging, and dark data cards.
- Reworked the responsive section with a cool gradient surface, a framed mobile product stage, and reduced empty-feeling space around the real mobile capture.
- Fixed the closing CTA inheriting the portfolio’s legacy 1000px section cap; it now spans the viewport on desktop and mobile.
- Rebuilt and verified the deployed page at 1920px and 390px: no horizontal overflow and all product images load.
...
- The blank right side at the closing CTA was not a browser rendering issue; it was the inherited global `section { max-width: 1000px; }` rule.

---

## 2026-09-23 — InsChat hero composition correction

### Solved

- Removed the hero’s three capability fact blocks and secondary source CTA.
- Shortened the hero copy to one promise, one sentence, and one live-demo action.
- Rebuilt the desktop hero as a true two-column composition with the copy on the left and the authentic InsChat screen directly beside it.
- Added a single-column responsive fallback for tablet and phone widths.
- Rebuilt and checked the page at desktop and 390px mobile widths with no horizontal overflow.
...
- The screenshot did not need to be removed; the problem was its relationship to the hero copy and the amount of competing text around it.

---

## 2026-09-23 — InsChat Muse layout restructure

### Solved

- Replaced the original two-column hero with a centered Muse-style introduction: one promise, one sentence, and one CTA.
- Added a separate full-width product-story section beneath the hero, with a centered narrative statement and the authentic InsChat screen as the main visual.
- Updated section navigation and numbering to follow the new introduction → product → health records → anywhere → boundaries → CTA sequence.
- Verified the rebuilt page with script-disabled browser rendering at desktop and 390px mobile widths; all images load and there is no horizontal overflow.
...
- A color change alone was not a Muse layout adaptation; the page needed a new centered hero and a dedicated product-introduction section.

---

## 2026-09-23 — InsChat warm-white product theme

### Solved

- Replaced the black-first InsChat presentation with warm white, soft blue, and neutral gray surfaces.
- Kept dark text and blue accents for contrast without placing white product captures on black backgrounds.
- Contained the product screenshots inside pale blue stages with tighter max widths, borders, and softer shadows.
- Restyled the workflow, build-notes, and CTA sections to maintain the warm-white visual system.
- Verified the deployed page with service-worker bypass at desktop and 390px mobile widths; images load and there is no horizontal overflow.
...
- The screenshot assets did not need to be replaced; the visual problem came from the dark surrounding theme and oversized screenshot treatment.

---

## 2026-09-23 — InsChat typography scale correction

### Solved

- Compared the rendered InsChat type scale with Muse’s live page: InsChat was using a 104px hero and 54px section headings, while Muse measured about 62px and 40px.
- Reduced InsChat’s desktop hero heading to 64px maximum and section headings to 40px maximum.
- Added smaller mobile caps while keeping the existing body copy readable.
- Verified the fresh rendered page at 1920px and 390px: the new heading sizes are applied, images load, and there is no horizontal overflow.
...
- The readability problem was not caused by the body font; the primary mismatch was the oversized heading scale.

---

## 2026-09-23 — InsChat Muse-inspired top bar behavior

### Solved

- Added a fixed top bar with InsChat branding on the left and an app action on the right.
- Added a centered InsChat monogram that stays hidden during the hero and fades in after the first viewport is passed.
- Kept the existing section menu and live/source actions available without copying Muse or Meta branding assets.
- Rebuilt successfully and verified the generated page contains the new top-bar markup.

### Unresolved

- The current browser session may retain an older JavaScript chunk through its service worker; a cache-bypassed reload is required to visually inspect the scroll transition.

### Disproved

- A static top bar alone would not reproduce the reference behavior; the centered mark needs scroll-aware visibility.

---

## 2026-09-23 — InsChat header treatment matched to reference

### Solved

- Inspected Muse’s live header DOM and measured its 60px transparent, borderless layout, centered container, and quiet elevated app button.
- Removed InsChat’s header border and initial backdrop so it merges naturally with the warm-white hero.
- Added a subtle backdrop only after scrolling past the hero.
- Added a centered InsChat hero monogram to mirror the reference page’s opening brand mark without copying its artwork.
- Rebuilt and verified the generated markup and top-bar rendering.

### Unresolved

- The browser session still needs a fresh non-service-worker-controlled runtime check for the scroll-triggered header transition.

### Disproved

- The earlier bordered, always-blurred header did not match the reference’s initial state; the reference relies on transparency and spacing first.

---

## 2026-09-23 — Removed unapproved InsChat icon treatment

### Solved

- Removed the invented hero `i` monogram.
- Removed the scroll-triggered center `i` icon and its scroll-state logic.
- Kept only the transparent branded top bar and app action.
- Rebuilt successfully and verified the generated page no longer contains the custom icon markup.
...
- A generic `i` glyph was not a valid substitute for the actual InsChat app icon.

---

## 2026-09-23 — InsChat real icon and arrowless actions

### Solved

- Retrieved the real InsChat app icon from the InsChat product source and added it to the showcase hero.
- Removed every generic upper-arrow glyph from InsChat’s hero, header, and closing actions.
- Kept the actions as clean text pills/buttons so they do not imply a copied external-link treatment.
- Rebuilt and verified the deployed markup: the real icon is present and no `↗` glyph remains in InsChat showcase components.
...
- A generic `i` glyph was not an acceptable app-icon substitute, and arrow glyphs were not needed to communicate the actions.

---

## 2026-09-23 — InsChat responsive device comparison

### Solved

- Replaced the single phone-only responsive visual with a real desktop screenshot plus the real mobile screenshot.
- Layered the phone frame over the desktop frame to make the responsive relationship immediately visible.
- Removed the large outer screenshot stage so the composition uses less space and the two devices become the focus.
- Verified the deployed section at desktop and 390px mobile widths; all images load and there is no horizontal overflow.
...
- A single phone screenshot was not enough evidence for the responsive claim; the desktop/mobile pair communicates the behavior more directly.

---

## 2026-09-23 — InsChat device bottom alignment

### Solved

- Anchored the desktop presentation to the same bottom edge as the phone presentation.
- Kept the overlap while removing the previous staggered baseline that made the two screenshots feel misaligned.
- Applied the same bottom anchoring at the mobile breakpoint.
- Rebuilt successfully and restarted the project preview with the new build.

### Unresolved

- Browser automation could not reach the local preview tab, but the preview server responded successfully on port 3000.

### Disproved

- Aligning the figure captions alone was insufficient; the screenshot frames themselves needed a shared bottom anchor.

---

## 2026-09-23 — InsChat General Chat showcase

### Solved

- Reverted the responsive desktop presentation to its previous staggered position after the shared-bottom alignment felt worse.
- Added a new General Chat section between the product introduction and Health Records sections.
- Added a dark, low-light General mode showcase with the real product language: General mode, the everyday assistant prompt, and the familiar composer actions.
- Added General Chat to the project navigation and renumbered the later section labels.
- Extracted the new visual into `InsChatGeneralChatVisual.js` so the main showcase component remains below the 300-line review threshold.
- Rebuilt successfully and restarted the project preview; the InsChat route responds with HTTP 200.

### Unresolved

- The repository does not contain a captured dark General Chat screenshot, so the new visual is a semantic showcase panel based on the live InsChat interface rather than a fabricated image asset.

### Disproved

- The shared-bottom device alignment did not improve the responsive composition; the original offset overlap is restored.

---

## 2026-09-23 — InsChat General Chat placement and palette

### Solved

- Moved General Chat from between Product and Health Records to the end of the showcase, immediately before the closing CTA.
- Restored the main section numbering so Health Records, Anywhere, and Boundaries remain the primary story flow.
- Changed the General Chat section from a full black/blue block to the warm-white page palette.
- Kept the dark theme inside the General Chat product window, where it communicates the feature without breaking the page rhythm.
- Rebuilt successfully, restarted the preview, and verified the InsChat route responds with HTTP 200.
...
- A full-bleed black section did not fit the warm-white showcase; the dark treatment works better as the contained General Chat interface.

---

## 2026-09-23 — InsChat General Chat section 04

### Solved

- Moved General Chat to section 04, after Health Records and before Anywhere and Built with boundaries.
- Updated later section labels to 05 / Anywhere and 06 / Built with boundaries.
- Reworked the visual to follow the live General mode shell: compact sidebar, General/New chat navigation, guest footer, centered welcome state, and the real InsChat app icon.
- Removed the invented Saved chats/sidebar treatment and the oversized dashboard-like composition.
- Rebuilt successfully, restarted the preview, and verified the InsChat route responds with HTTP 200.

### Unresolved

- The live dark interface is not available as a repository screenshot asset, so the visual is a faithful HTML rendition of the observed live layout rather than a copied image.

### Disproved

- The previous custom dark dashboard did not represent InsChat’s actual General mode layout and was removed.

---

## 2026-09-24 — InsChat project link canonical form

### Solved

- Updated the InsChat project card link from `/project/inschat/` to `/project/inschat` as requested.
- Rebuilt with Node 16 after the default Node version hit Gatsby’s OpenSSL 3 hash incompatibility.
- Restarted the preview with the successful build; the no-slash route responds and Gatsby redirects it to the generated trailing-slash page route.

### Unresolved

- Gatsby’s generated page route still canonicalizes to a trailing slash at the server level; the project card now emits the requested no-slash URL.

### Disproved

- Building with the default Node version was not reliable for this Gatsby 3 project because of `error:0308010C:digital envelope routines::unsupported`.

---

## 2026-09-24 — InsChat General Chat real capture

### Solved

- Removed the hand-built General Chat HTML mock from the showcase.
- Added a capture from the live InsChat General mode screen and displayed it as the section’s product visual.
- Made the General Chat band full width so its centered 1240px inner grid matches the other showcase sections.

### Unresolved

- The locally transferred live capture is a 320×240 browser image, so a higher-resolution source capture would improve sharpness if one is supplied or made transferable.

### Disproved

- Recreating the sidebar, welcome state, and composer with portfolio HTML did not satisfy the requirement to show the real product interface.

---

## 2026-09-24 — InsChat General Chat Light capture

### Solved

- Captured the real General mode from the local InsChat app with its Light theme enabled.
- Replaced the dark General Chat asset and removed the dark presentation background.
- Preserved the full-width section grid and real product screenshot treatment.

### Unresolved

- None for the requested dark-theme removal.

### Disproved

- The dark General mode capture was incompatible with the warm-white showcase direction, even though it represented the real product.

---

## 2026-09-24 — InsChat section 02 latest Health chat capture

### Solved

- Authenticated to the live InsChat app, enabled Health mode, and opened the newest `9/24 Dinner` session.
- Captured the real conversation, food impact summary, and glucose reading in the Light theme.
- Replaced only section 02’s product screenshot and kept the existing desktop landing capture for the responsive section.

### Unresolved

- None for the requested section 02 screenshot replacement.

### Disproved

- Reusing the Health mode landing screen did not demonstrate the completed chat workflow the section describes.

---

## 2026-09-24 — InsChat section 02 full-screen capture

### Solved

- Refreshed the authenticated InsChat session after the account’s record cleanup.
- Collapsed the General chat folder before capturing the latest Health conversation.
- Replaced the section 02 asset with a full 1440×900 Light-theme app capture to reduce the apparent zoom while preserving detail.

### Unresolved

- None for the requested full-screen framing.

### Disproved

- The earlier 1024×720 capture made the conversation feel too tightly framed when presented in the showcase.

---

## 2026-09-24 — InsChat section 02 supplied image restoration

### Solved

- Restored the supplied food image into the authenticated session’s browser-local IndexedDB using its existing message key.
- Reloaded the latest `9/24 Dinner` chat and captured the image together with the real Health summary.
- Kept the General folder collapsed and preserved the 1440×900 full-screen framing.

### Unresolved

- The image is available in the local capture browser only; the app still intentionally treats it as browser-local data.

### Disproved

- A text-and-record-only capture was insufficient to demonstrate the image-based Health workflow.

---

## 2026-09-24 — InsChat section 02 refreshed capture

### Solved

- Re-ran the capture against the current latest `9/24 Dinner` session after the page data changed.
- Restored the supplied food image under the current session’s existing image key.
- Captured the updated four-message conversation at 1440×900 with General collapsed.

### Unresolved

- None for the requested recapture.

### Disproved

- Reusing the previous section 02 asset would have shown stale conversation content.

---

## 2026-09-24 — InsChat section 04 General chat capture

### Solved

- Opened the authenticated account’s completed General chat comparing GPT-6 Luna and DeepSeek pricing.
- Captured the real Light-theme conversation at 1440×900.
- Collapsed Health mode so the General chat remains the visual focus.
- Replaced section 04’s empty-state screenshot with the completed conversation capture.

### Unresolved

- None for the requested General chat screenshot.
